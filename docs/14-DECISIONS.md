...
| DEC-015 | 2026-09-12 | **Remplacée par DEC-020 (2026-09-13)** | Inscription libre (self-service) au lieu de création par Signa après paiement | Permettre de construire et tester le noyau multi-organisation sans attendre le flux de paiement complet (Phase 0) |
...
| DEC-020 | 2026-09-13 | Acceptée | Retrait de l'inscription libre : seul le personnel Signa peut créer une organisation (via `admin_create_organization_with_invitation`, invitation par lien) | Tranche DEC-015/DEC-003 dans le sens du MVP documenté (« Signa crée le compte après paiement »). Politique RLS `organizations_insert_authenticated` (n'importe quel utilisateur authentifié) remplacée par `organizations_insert_staff_admin` (`is_internal_admin()` seulement) — migration `20260913000000` |
