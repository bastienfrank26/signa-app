// Faux "browser window" mockup used in the hero, before/after, and project gallery.
// Stylized — we don't render real product UIs. Photos stay as labeled placeholders.
...
const SiteMockup = ({ brand, title, cta = 'Demander une soumission', variant = 'modern', className = '', photoLabel = 'photo client' }: MockupProps) => {
    ...
    <div className={`relative overflow-hidden rounded-xl ${legacy ? 'bg-[#d9d2c2]' : 'bg-ink-800'} shadow-card ${className}`}>
      ...
      <div className={`flex items-center justify-between px-4 py-2 text-[10px] ${legacy ? 'text-[#3a3528] bg-[#e9e1cf]' : 'text-white/90 bg-ink-700/40'}`}>
        ...
        <div className="flex items-center gap-3 opacity-80">
          ...
          {!legacy && <span className="ml-1 inline-block px-2 py-0.5 rounded bg-brand text-white text-[9px] font-semibold">Soumission</span>}
        </div>
      </div>
      ...
      <div className="relative aspect-[16/10]">
        ...
        {!legacy && <div className="absolute inset-0 bg-gradient-to-t from-ink-900/80 via-ink-900/30 to-ink-900/10"></div>}
        <div className="absolute inset-0 flex items-center justify-center">
          <span className={`text-[9px] uppercase tracking-[0.25em] ${legacy ? 'text-[#5a4f3a]' : 'text-white/40'}`}>{photoLabel}</span>
        </div>
...
(window as any).SiteMockup = SiteMockup;
